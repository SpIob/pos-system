...
public class SalesPanel extends JPanel {
    ...
    public void setCurrentUser(model.User user) { this.currentUser = user; }
    ...
    private JLabel           totalLabel, changeLabel;
    private JTextField       amountPaidField;
    private JButton          chargeButton, clearCartButton;
    ...
    public void loadProducts() {
        new SwingWorker<java.util.List<model.Product>, Void>() {
            @Override protected java.util.List<model.Product> doInBackground() {
            ...
            @Override protected void done() {
                try {
                    ...
                    for (model.Product p : products)
                        productsGridPanel.add(buildProductButton(p.getProductName(),
                            String.format("%.2f", p.getPrice()), p.getProductId()));
    ...
    private JButton buildProductButton(String name, String price, int productId) {
        ...
        btn.setBackground(Color.WHITE); btn.setOpaque(false); btn.setContentAreaFilled(false);
        btn.setBorderPainted(false); btn.setFocusPainted(false);
        btn.setPreferredSize(new Dimension(0, 72));
        btn.setCursor(new Cursor(Cursor.HAND_CURSOR));
        btn.addMouseListener(new MouseAdapter() {
            @Override public void mouseEntered(MouseEvent e) { btn.setBackground(UIHelper.ROW_ALT); btn.repaint(); }
            @Override public void mouseExited(MouseEvent e)  { btn.setBackground(Color.WHITE); btn.repaint(); }
            @Override public void mouseClicked(MouseEvent e) { addToCart(name, price); }
        });
        return btn;
    }
    ...
    private JScrollPane buildOrderTable() {
        orderModel = new DefaultTableModel(new String[]{"Item", "Qty", "Unit Price", "Subtotal"}, 0) {
    ...
    private void addToCart(String name, String priceStr) {
        ...
        orderModel.addRow(new Object[]{name, 1,
            String.format("\u20B1%.2f", price), String.format("\u20B1%.2f", price)});
        updateTotal();
    }
    ...
    private void updateChange() {
        try {
            double total = parseTotal(), paid = Double.parseDouble(amountPaidField.getText().trim());
    ...
    private void handleCharge() {
        if (orderModel.getRowCount() == 0) {
            JOptionPane.showMessageDialog(this, "Please add items to the order first.",
                "Empty Order", JOptionPane.WARNING_MESSAGE); return;
        }
        double total, paid;
        try {
            total = parseTotal(); paid = Double.parseDouble(amountPaidField.getText().trim());
        } catch (NumberFormatException ex) {
            JOptionPane.showMessageDialog(this, "Please enter a valid amount paid.",
                "Invalid Amount", JOptionPane.WARNING_MESSAGE); return;
        }
        if (paid < total) {
            JOptionPane.showMessageDialog(this, "Amount paid is less than the total.",
                "Insufficient Payment", JOptionPane.WARNING_MESSAGE); return;
        }

        int userId = (currentUser != null) ? currentUser.getUserId() : 1;
        model.Transaction txn = new model.Transaction();
        txn.setUserId(userId); txn.setTotalAmount(total);
        txn.setAmountPaid(paid); txn.setChangeGiven(paid - total);
        ...
        for (int i = 0; i < orderModel.getRowCount(); i++) {
            ...
            double unitPrice = Double.parseDouble(orderModel.getValueAt(i, 2).toString()
                              .replace("\u20B1", "").replace(",", ""));
            model.TransactionItem item = new model.TransactionItem();
            item.setProductId(productIdMap.getOrDefault(itemName, 0));
            item.setItemDescription(itemName); item.setQuantity(qty);
            item.setUnitPrice(unitPrice); item.setSubtotal(qty * unitPrice);
            items.add(item);
            receiptLines.append(itemName).append(" x").append(qty)
                        .append("  \u20B1").append(String.format("%.2f", qty * unitPrice)).append("\n");
        }

        final double finalTotal = total, finalPaid = paid;
        final String finalLines = receiptLines.toString();
        chargeButton.setEnabled(false); chargeButton.setText("Processing...");
        ...
        new SwingWorker<Integer, Void>() {
            ...
            @Override protected void done() {
                chargeButton.setEnabled(true); chargeButton.setText("CHARGE");
                try {
                    int newTxnId = get();
                    if (newTxnId < 0) {
                        JOptionPane.showMessageDialog(SalesPanel.this,
                            "Transaction could not be saved. Check your connection.",
                            "Save Error", JOptionPane.ERROR_MESSAGE); return;
                    }
                    new ReceiptDialog(null, String.format("TXN-%04d", newTxnId),
    ...
    private void loadStationDropdown() {
        java.util.List<model.Station> available = new dao.StationDAO().getAvailableStations();
        stationComboBox.removeAllItems();
        stationComboBox.addItem("Select station...");
        for (model.Station s : available) stationComboBox.addItem(s.getStationName());
    }
