...
public class ReportsPanel extends JPanel {
    ...
    private JLabel todaysSalesValue, totalTxnValue, lowStockValue;
    private DefaultTableModel txnModel, inventoryModel;
    private JTable txnTable, inventoryTable;
    ...
    private JButton exportBtn, voidBtn;
    ...
    private JPanel buildStatsRow() {
        ...
        Font bigFont = new Font("Dialog", Font.BOLD, 26);
        todaysSalesValue.setFont(bigFont); totalTxnValue.setFont(bigFont); lowStockValue.setFont(bigFont);

        row.add(salesCard); row.add(txnCard); row.add(lowCard);
        return row;
    }
    ...
    private JPanel buildTransactionsPanel() {
        ...
        voidBtn.setEnabled(false); voidBtn.setVisible(false);
        voidBtn.addMouseListener(new MouseAdapter() {
            @Override public void mouseClicked(MouseEvent e) { if (voidBtn.isEnabled()) handleVoidTransaction(); }
        });
        ...
        adminBtns.add(voidBtn); adminBtns.add(exportBtn);
        titleRow.add(title, BorderLayout.WEST);
        titleRow.add(adminBtns, BorderLayout.EAST);

        txnModel = new DefaultTableModel(new String[]{"TXN #", "Date & Time", "Items", "Total", "Cashier", "Status"}, 0) {
        ...
        txnTable.getSelectionModel().addListSelectionListener(e -> {
            ...
            if (sel >= 0 && currentUser != null && currentUser.isAdmin()) {
                String status = txnStatusList.size() > sel ? txnStatusList.get(sel) : "completed";
                voidBtn.setEnabled("completed".equals(status));
            } else { voidBtn.setEnabled(false); }
        });
        ...
        txnTable.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
            @Override public java.awt.Component getTableCellRendererComponent(
                ...
                boolean isVoided = row < txnStatusList.size() && "voided".equals(txnStatusList.get(row));
                if (sel) { setBackground(UIHelper.NAVY); setForeground(Color.WHITE); }
                else if (isVoided) { setBackground(UIHelper.ROW_VOID); setForeground(new Color(0x999999)); }
                else {
        ...
        txnTable.getColumnModel().getColumn(5).setCellRenderer(UIHelper.statusRenderer("Completed", "Voided"));
    ...
    private JPanel buildInventoryPanel() {
        ...
        inventoryModel = new DefaultTableModel(new String[]{"Product", "Stock", "Status"}, 0) {
            @Override public boolean isCellEditable(int r, int c) { return false; }
        };
        inventoryTable = new JTable(inventoryModel);
        UIHelper.styleTable(inventoryTable);
        inventoryTable.getColumnModel().getColumn(2).setCellRenderer(UIHelper.statusRenderer("✔ OK", "⚠ LOW"));
        ...
        inventoryTable.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
            @Override public java.awt.Component getTableCellRendererComponent(
                ...
                if (sel) { setBackground(UIHelper.NAVY); setForeground(Color.WHITE); }
                    ...
                    String status = inventoryModel.getValueAt(row, 2).toString();
                    setBackground("LOW".equals(status) ? UIHelper.ROW_LOW
    ...
    public void refreshData() {
        new SwingWorker<Void, Void>() {
            double todayTotal; int todayCount, lowStockCount;
            ...
            @Override protected void done() {
                ...
                txnModel.setRowCount(0); txnIdList.clear(); txnStatusList.clear();
                for (Object[] row : richRows) {
                    int txnId = (int) row[0];
                    java.sql.Timestamp date = (java.sql.Timestamp) row[1];
                    int itemCount = (int) row[2]; double total = (double) row[3];
                    String username = (String) row[4], status = (String) row[5];
                    txnIdList.add(txnId);
                    txnStatusList.add(status != null ? status : "completed");
                    txnModel.addRow(new Object[]{String.format("TXN-%04d", txnId), fmt.format(date),
                ...
                for (Product p : allProducts)
                    inventoryModel.addRow(new Object[]{p.getProductName(), p.getStockQuantity(),
                        p.isLowStock() ? "LOW" : "OK"});
            }
        }.execute();
    }
    ...
    private void handleVoidTransaction() {
        ...
        PasswordConfirmDialog pwd = new PasswordConfirmDialog(null, currentUser, "Void " + txnNum);
        pwd.setVisible(true);
        if (!pwd.isConfirmed()) return;
        int choice = JOptionPane.showConfirmDialog(this,
            "Void " + txnNum + "?\n\nStock quantities will be restored.\nThis action cannot be undone.",
            "Confirm Void Transaction", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
        if (choice != JOptionPane.YES_OPTION) return;
        new SwingWorker<Boolean, Void>() {
            @Override protected Boolean doInBackground() { return transactionDAO.voidTransaction(txnId); }
            @Override protected void done() {
                try {
                    if (get()) {
                ...
                try (FileWriter fw = new FileWriter(file)) {
                    ...
                    for (Object[] row : rows) {
                        fw.write(String.format("TXN-%04d,%s,%d,%.2f,%s,%s\n",
                            (int) row[0], fmt.format((java.sql.Timestamp) row[1]),
                            (int) row[2], (double) row[3],
                            row[4] != null ? row[4] : "",
                            row[5] != null ? row[5] : "completed"));
                    }
                    JOptionPane.showMessageDialog(ReportsPanel.this,
