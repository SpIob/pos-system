...
public class ReceiptDialog extends JDialog {
    ...
    private void buildUI(String txnNumber, String itemLines,
        ...
        JLabel icon;
        if (logoUrl != null) {
            java.awt.Image img = new javax.swing.ImageIcon(logoUrl)
                    .getImage().getScaledInstance(40, 40,
                            java.awt.Image.SCALE_SMOOTH);
            icon = new JLabel(new javax.swing.ImageIcon(img),
        ...
        // Café name
        JLabel name = new JLabel("ByteZone Café POS",
                SwingConstants.CENTER);
        ...
        // Address
        JLabel addr = new JLabel("Quezon City, Metro Manila",
                SwingConstants.CENTER);
        ...
        // Receipt body
        receiptArea = new JTextArea(buildReceiptText(
                txnNumber, itemLines, total, amountPaid));
        receiptArea.setFont(
    ...
    private String buildReceiptText(String txnNumber,
            String itemLines, double total, double amountPaid) {
        String date = new SimpleDateFormat(
                "MMMM d, yyyy   hh:mm a").format(new Date());
        ...
        for (String line : itemLines.split("\n")) {
            if (!line.trim().isEmpty())
                sb.append(padLine(line)).append("\n");
        }

        sb.append("\n").append(dashes()).append("\n");
        sb.append(String.format("%-18s %10s%n",
    ...
    private String padLine(String line) {
        ...
        String left  = line.substring(0, last).trim();
        String right = line.substring(last).trim();
        int    dots  = Math.max(1, 30 - left.length() - right.length());
