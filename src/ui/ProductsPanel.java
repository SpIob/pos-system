...
public class ProductsPanel extends JPanel {
    ...
    private JPanel buildToolbar() {
        ...
        searchField.addFocusListener(new FocusAdapter() {
            @Override public void focusGained(FocusEvent e) {
                if (searchField.getText().startsWith("🔍")) {
                    searchField.setText(""); searchField.setForeground(new Color(0x333333));
                }
            }
        ...
        addBtn.addMouseListener(new MouseAdapter() {
            @Override public void mouseClicked(MouseEvent e) { if (addBtn.isEnabled()) openAddDialog(); }
        });
        editBtn.addMouseListener(new MouseAdapter() {
            @Override public void mouseClicked(MouseEvent e) { if (editBtn.isEnabled()) openEditDialog(); }
        });
        deleteBtn.addMouseListener(new MouseAdapter() {
            @Override public void mouseClicked(MouseEvent e) { if (deleteBtn.isEnabled()) deleteSelected(); }
        });
        ...
        lowStockBanner = new JPanel(new BorderLayout()) {
            @Override protected void paintComponent(Graphics g) {
                super.paintComponent(g); g.setColor(UIHelper.AMBER);
    ...
    private JScrollPane buildTable() {
        productModel = new DefaultTableModel(
                new String[]{"Product Name", "Category", "Price", "Stock", "Threshold", "Status"}, 0) {
        ...
        productTable.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
            @Override public java.awt.Component getTableCellRendererComponent(
                ...
                if (sel) {
                    setBackground(UIHelper.NAVY); setForeground(Color.WHITE);
                } else {
                    int mRow = productTable.convertRowIndexToModel(row);
                    String status = productModel.getValueAt(mRow, 5).toString();
                    setBackground(status.equals("LOW") ? UIHelper.ROW_LOW
    ...
    public void refreshData() {
        new SwingWorker<Void, Void>() {
            List<Product> products, lowStock;
            ...
            @Override protected void done() {
                productModel.setRowCount(0); productIdList.clear();
                for (Product p : products) {
                    productIdList.add(p.getProductId());
                    productModel.addRow(new Object[]{
                        p.getProductName(), UIHelper.capitalize(p.getCategory()),
                        String.format("\u20B1%.2f", p.getPrice()), p.getStockQuantity(),
                        GlobalSettings.LOW_STOCK_THRESHOLD, p.isLowStock() ? "LOW" : "OK"});
                }
                if (lowStock.isEmpty()) {
                    lowStockBanner.setVisible(false);
                } else {
                    ...
                    for (int i = 0; i < lowStock.size(); i++) {
                        ...
                        msg.append(p.getProductName()).append(" (").append(p.getStockQuantity()).append(" remaining)");
    ...
    private void openAddDialog() {
        ...
        dlg.setOnSaveListener(data -> {
            ...
            p.setProductName(data[0]); p.setCategory(data[1].toLowerCase());
            p.setPrice(Double.parseDouble(data[2])); p.setStockQuantity(Integer.parseInt(data[3]));
            p.setLowStockThreshold(GlobalSettings.LOW_STOCK_THRESHOLD);
            if (productDAO.addProduct(p)) refreshData();
            else JOptionPane.showMessageDialog(ProductsPanel.this,
                "Failed to save product. Check your connection.", "Save Error", JOptionPane.ERROR_MESSAGE);
        });
        dlg.setVisible(true);
    }
    ...
    private void openEditDialog() {
        ...
        AddProductDialog dlg = new AddProductDialog(null, new String[]{name, category, price, stock});
        dlg.setOnSaveListener(data -> {
            Product p = new Product();
            p.setProductId(pid); p.setProductName(data[0]); p.setCategory(data[1].toLowerCase());
            p.setPrice(Double.parseDouble(data[2])); p.setStockQuantity(Integer.parseInt(data[3]));
            p.setLowStockThreshold(GlobalSettings.LOW_STOCK_THRESHOLD);
            if (productDAO.updateProduct(p)) refreshData();
            else JOptionPane.showMessageDialog(ProductsPanel.this,
                "Failed to update. Check your connection.", "Update Error", JOptionPane.ERROR_MESSAGE);
        });
        dlg.setVisible(true);
    }
    ...
    private void deleteSelected() {
        ...
        if (choice == JOptionPane.YES_OPTION) {
            if (productDAO.deleteProduct(pid)) refreshData();
    ...
    private void applyFilter() {
        ...
        sorter.setRowFilter((text.isEmpty() || text.startsWith("🔍"))
                ? null : RowFilter.regexFilter("(?i)" + text, 0));
    }

    private int getProductIdAtRow(int modelRow) {
        return (modelRow >= 0 && modelRow < productIdList.size()) ? productIdList.get(modelRow) : -1;
    }
}